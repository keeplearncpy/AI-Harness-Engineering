...
## Workflow
1. **实体提取**: 阅读所有 FSD 文档，提取所有数据实体和属性
2. **关系建模**: 定义实体间关系（1:1, 1:N, N:M）、外键约束
3. **字段定义**: 为每个实体定义列名、类型、约束、默认值、索引
4. **命名规范**: 统一使用 snake_case 命名，表名用复数形式
5. **生成 DDL**: 按 PostgreSQL 语法生成完整建表语句，包含注释
6. **绘制 ER 图**: 用 Mermaid 语法生成实体关系图
7. **编写字典**: 为每个字段编写数据字典条目
8. **自检清单**: 对照 `skills/checklists/db_checklist.md` 逐条验证
